    ...
    for i,sent in enumerate(data):
        ...
        for word in words:
            if word not in translation_dictionary:
                continue
            h=int(hashlib.sha224(word.encode("utf-8")).hexdigest(), 16)%sparse_size # TODO: replace this with word2index dictionary
            row.append(i)
            col.append(h)
...
def load_data(lang_fname):
    ...
    vectors=vectors.reshape(int(len(src_vectors)/150),150) #[:10100,:]
...
def process_metadata(m):
    ...
    for i,row in enumerate(m):
        cluster,length=row
        if cluster not in d: # this is new cluster, so mark start point and end point for last cluster
            if last!=None:
                d[last][1]=i-1 # end point
                last=None
            d[(cluster,length)]=(i,None) # start point
            last=(cluster,length)
    else:
        if last:
            d[last][1]=i # end point
            
    return d
    
def align_slices(src,trg,limit_length=(1,6)):
    ...
    for key,value in src.items():
        ...
        aligned_trg[key]=(trg[(key[0],key[1]+limit_length[0])],trg[(key[0],key[1]+limit_length[1])])
    return aligned_trg
...
def get_slices(src_m,trg_m):
    ...
    aligned_trg=align_slices(src,trg)
    return src,aligned_trg

def rank(src_fname,trg_fname,outfname):
    ...
    f2e_dictionary=build_dictionary(args.dictionary+".f2e",args.dict_vocabulary+"fi")
    e2f_dictionary=build_dictionary(args.dictionary+".e2f",args.dict_vocabulary+"en")
    
    # build target sparse matrices    
    trg_sparse,trg_translated_sparse,trg_normalizer=build_sparse_matrices(trg_sentences,e2f_dictionary)
    trg_sparse=trg_sparse.tocsc() # csc # TODO: we have to slice this so how csc works???
    trg_translated_sparse=trg_translated_sparse.tocsc()
    
    # build source sparse matrices
    src_sparse,src_translated_sparse,src_normalizer=build_sparse_matrices(src_sentences,f2e_dictionary)
    src_sparse=src_sparse.tocsr() # csr
    src_translated_sparse=src_translated_sparse.tocsr()
    
        
        
    ## TODO: continue from here (rest is still untouched, nothing is tested...)!!!
